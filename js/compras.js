 ...
 * compras.js — Módulo de Compras (reestruturado)
 */
...
function _renderDashCompras() {
  ...
  el.innerHTML = `
      ...
      ${la ? `
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
          <div style="text-align:center;padding:6px 14px;background:var(--surface2);border-radius:var(--r8);border:1px solid var(--border)">
            ...
            <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Itens</div>
          </div>
          <div style="text-align:center;padding:6px 14px;background:var(--surface2);border-radius:var(--r8);border:1px solid var(--border)">
            <div style="font-size:.88rem;font-weight:800;color:var(--purple)">R$${fmt(la.valorEstimado)}</div>
            <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Estimado</div>
          </div>
          <div style="text-align:center;padding:6px 14px;background:${st.bg || 'var(--surface2)'};border-radius:var(--r8);border:1px solid ${st.color || 'var(--border)'}">
            ...
            <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Status</div>
          </div>
    ...
    <div style="margin-top:14px;display:flex;gap:3px">
      ${[
        { n: 1, label: 'Lista',       icon: 'clipboard-list' },
        { n: 2, label: 'Aprovação',   icon: 'check-circle'   },
        { n: 3, label: 'Ordem',       icon: 'shopping-bag'   },
        { n: 4, label: 'Recebimento', icon: 'package'        },
      ].map(s => {
...
// ══════════════════════════════════════════════════════════════
// ETAPA 1 — LISTA DE COMPRAS COM COTAÇÕES
// ══════════════════════════════════════════════════════════════
function _renderEtapa1() {
  ...
  const prazoHtml = l.prazoCotacao ? `
    <div style="display:flex;align-items:center;gap:10px;padding:9px 14px;background:var(--yellow-light);
      ...
      <span>Prazo de cotação: <strong>${fmtDT(l.prazoCotacao)}</strong></span>
  ...
  document.getElementById('comprasContent').innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
        ${['montagem','cotacao','cotacao_encerrada'].map(s => {
          ...
          return `<button onclick="setStatusLista('${s}')"
            ...
            background:${isActive ? st.bg : 'var(--surface)'};
            color:${isActive ? st.color : 'var(--muted)'};cursor:pointer;transition:all .15s">${st.label}</button>`;
        }).join('')}
        <div style="width:1px;height:20px;background:var(--border)"></div>
        <button class="btn btn-outline btn-sm" onclick="abrirPrazoCotacao()">${lc('clock', 13, 'currentColor')} Prazo</button>
        <button class="btn btn-wa btn-sm" onclick="enviarTodasCotacoesWA()">${lc('message-circle', 13, '#fff')} Cotar via WA</button>
      </div>
    </div>
    ...
    <div class="card" style="margin-bottom:16px;overflow:hidden">
      <div style="padding:10px 14px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;background:var(--surface2)">
        ...
        <div style="font-size:.67rem;color:var(--muted)">${lc('info', 12, 'var(--muted)')} Adicione fornecedores por item para cotar</div>
      </div>
      <div class="tbl-wrap" style="border:none">
        <table>
          <thead><tr>
            ...
            <th class="r" style="width:110px">Ref. estimado</th>
    ...
    <div class="field" style="margin-bottom:20px">
      ...
      <textarea class="inp" rows="2" placeholder="Condições de pagamento, urgência, instruções..." onchange="setObsLista(this.value)">${l.observacoes || ''}</textarea>
    </div>
...
// ── Linhas de item + sublinhas de cotação ─────────────────────
  ...
  const mainRow = `<tr id="item1-${i.id}" style="background:var(--surface);border-bottom:${cotacoes.length ? 'none' : '1px solid var(--border)'}">
    <td style="padding:10px 14px">
      ...
      <div style="font-size:.63rem;color:var(--muted);margin-top:2px">
        ${lc(i.origem === 'manual' ? 'edit-2' : 'package', 11, 'var(--muted)')}
        ${i.categoria}${i.origem === 'manual' ? ' · Manual' : ''}
      </div>
    </td>
    <td class="c">
      ...
      <td style="padding:6px 10px">
        ${sup?.phone ? `
          <a href="https://wa.me/55${(sup.phone || '').replace(/\D/g,'')}?text=${encodeURIComponent(_montaMsgCotacaoForn(sup, i))}" target="_blank"
            style="display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:50%;background:#25D366;color:#fff;text-decoration:none" title="Enviar WA">
            ${lc('message-circle', 13, '#fff')}
          </a>
        ` : ''}
      </td>
      <td class="c">
        <button onclick="removerCotacao(${i.id},${idx})" style="background:none;border:none;color:var(--muted);cursor:pointer;padding:4px" title="Remover fornecedor">
...
function abrirAddFornecedor(itemId) {
  ...
  const sel = disponiveis.map(s =>
    `<option value="${s.id}">${s.name}${s.phone ? '' : ' ⚠️ sem telefone'}</option>`
  ).join('');
...
function enviarTodasCotacoesWA() {
  ...
  sups.forEach(({ sup }) => {
    const msg = _montaMsgCotacaoForn(sup, null);
...
function saveItemManual() {
  ...
  _listaAtual.itens.push({
    ...
    fornecedorId: null,
    localCompra:         localEl?.value || '',
    diaCompra: '', responsavelCompra: '',
    ...
    <div style="background:white;border-radius:var(--r14);padding:22px;width:100%;max-width:340px;box-shadow:0 12px 40px rgba(0,0,0,.2)">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
        <div class="field" style="margin:0">
          <label>Data</label>
          <input type="date" id="prazoData" class="inp" value="${dataVal}">
        </div>
        <div class="field" style="margin:0">
          <label>Horário</label>
      ...
      ${atual ? `
        <button onclick="limparPrazo()"
          style="width:100%;padding:6px;border:1.5px solid var(--red);border-radius:var(--r6);
          background:var(--red-light);color:var(--red);font-size:.74rem;font-weight:600;cursor:pointer;margin-bottom:10px">
          ${lc('x', 12, 'currentColor')} Remover prazo
        </button>` : ''}
      <div style="display:flex;gap:8px;justify-content:flex-end">
...
function _renderEtapa2() {
  ...
  const apWa = l.aprovadorWa
    ? `https://wa.me/55${l.aprovadorWa.replace(/\D/g,'')}?text=${encodeURIComponent(`Lista ${l.codigo} aguardando sua aprovação no sistema Vai Ter Pizza!`)}`
    : null;
  ...
  document.getElementById('comprasContent').innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;flex-wrap:wrap;gap:10px">
      <div>
        ...
        <div style="font-size:.71rem;color:var(--muted)">
          ${l.aprovadorNome ? `Aprovador: <strong>${l.aprovadorNome}</strong>` : 'Revise e aprove cada item antes da compra'}
        </div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:10px;margin-bottom:16px">
      <div style="background:var(--purple-xlight);border:1.5px solid var(--purple-light);border-radius:var(--r10);padding:10px 14px;text-align:center">
        ...
        <div style="font-size:.6rem;color:var(--muted);text-transform:uppercase">Total estimado</div>
      </div>
    ...
    ${!todoRespondido ? `
    <div style="display:flex;align-items:center;gap:8px;padding:9px 14px;background:var(--yellow-light);
      border:1.5px solid var(--yellow);border-radius:var(--r8);margin-bottom:14px;font-size:.74rem;color:var(--orange-dark);font-weight:600">
      ${lc('alert-triangle', 14, 'var(--yellow)')} Responda todos os itens para habilitar a aprovação final
    </div>` : ''}

    ${itensForn.length ? `
      <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:var(--muted);margin-bottom:8px;display:flex;align-items:center;gap:5px">
        ${lc('building-2', 12, 'var(--muted)')} Compras via Fornecedor
      </div>
    ...
    ${itensPresencial.length ? `
      <div style="font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.7px;color:var(--muted);margin-bottom:8px;display:flex;align-items:center;gap:5px">
        ${lc('shopping-cart', 12, 'var(--muted)')} Compra Presencial
      </div>
      <div style="background:var(--orange-light);border:1.5px solid var(--border);border-radius:var(--r8);
        padding:9px 12px;margin-bottom:8px;font-size:.72rem;color:var(--orange-dark)">
        ${lc('info', 12, 'var(--orange-dark)')} Itens sem fornecedor. Aprovação por <strong>orçamento máximo</strong>.
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:20px">
...
function _cardAprovacaoItem(i, isPresencial) {
  ...
  return `<div style="border:1.5px solid ${isAprov?'var(--green)':isReprov?'var(--red)':'var(--border)'};
    border-radius:var(--r10);background:${isAprov?'var(--green-light)':isReprov?'var(--red-light)':'var(--surface)'};overflow:hidden;transition:all .2s">
    <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;flex-wrap:wrap">
      ...
      <div style="flex:1;min-width:140px">
        ...
        <div style="font-size:.66rem;color:var(--muted);margin-top:2px">
          ${isPresencial
            ? `${lc('shopping-cart',11,'var(--muted)')} Compra presencial`
            : `${lc('building-2',11,'var(--muted)')} ${sup?.name || 'Fornecedor'}`}
        </div>
      </div>
      <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap">
        <div style="text-align:center">
          <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase">Qtd</div>
          <div style="font-size:.82rem;font-weight:700;font-family:monospace">${fmt(qtd)} ${i.unidade}</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase">${isPresencial ? 'Orç. máx.' : 'Preço unit.'}</div>
        ...
        <div style="text-align:center">
          <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase">Total</div>
        ...
        <div style="display:flex;gap:5px">
          <button onclick="aprovarItem2(${i.id})"
            style="padding:6px 13px;border-radius:var(--r8);border:1.5px solid ${isAprov?'var(--green)':'var(--border)'};
            background:${isAprov?'var(--green)':'var(--surface)'};color:${isAprov?'#fff':'var(--text2)'};
            font-size:.72rem;font-weight:600;cursor:pointer;transition:all .15s;display:flex;align-items:center;gap:4px">
            ${lc('check', 12, 'currentColor')} Aprovar
          </button>
          <button onclick="reprovarItem2(${i.id})"
            style="padding:6px 13px;border-radius:var(--r8);border:1.5px solid ${isReprov?'var(--red)':'var(--border)'};
            background:${isReprov?'var(--red)':'var(--surface)'};color:${isReprov?'#fff':'var(--text2)'};
            font-size:.72rem;font-weight:600;cursor:pointer;transition:all .15s;display:flex;align-items:center;gap:4px">
            ${lc('x', 12, 'currentColor')} Reprovar
          </button>
        </div>
      </div>
    </div>
    <div style="padding:0 14px 10px 20px">
      <input type="text" placeholder="Comentário do aprovador (opcional)..." value="${i.comentarioAprovador || ''}"
  ...
  popup.innerHTML = `
    <div style="background:white;border-radius:var(--r14);padding:22px;width:100%;max-width:340px;box-shadow:0 12px 40px rgba(0,0,0,.2)">
      ...
      <div class="field">
        ...
        <input type="text" id="apNome" class="inp" placeholder="Ex: João (Gerente)" value="${l.aprovadorNome || ''}">
      </div>
      <div class="field">
        <label>WhatsApp do aprovador</label>
        <input type="tel" id="apWa" class="inp" placeholder="Ex: 11999990000" value="${l.aprovadorWa || ''}">
      </div>
...
function _renderEtapa3() {
  ...
  document.getElementById('comprasContent').innerHTML = `
    ...
    ${Object.entries(bySup).map(([supId, itens], idx) => {
      ...
      const statusOC = itens[0]?.statusOC || 'pendente';
      const stOCColors = { pendente:'var(--muted)', enviada:'var(--yellow)', confirmada:'var(--green)' };
      const stOCLabels = { pendente:'Não enviada', enviada:'Enviada', confirmada:'Confirmada' };

      return `<div class="card" style="margin-bottom:14px;overflow:hidden">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;
          background:var(--purple-xlight);border-bottom:1.5px solid var(--purple-light);flex-wrap:wrap;gap:8px">
          <div>
          ...
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            <span style="font-size:.67rem;font-weight:700;color:${stOCColors[statusOC]};
              padding:3px 8px;border-radius:20px;border:1px solid ${stOCColors[statusOC]};background:white">
              ${lc(statusOC==='confirmada'?'check-circle':statusOC==='enviada'?'send':'clock', 11, 'currentColor')} ${stOCLabels[statusOC]}
            </span>
        ...
        ${itens.map((i, iIdx) => `
          <div style="display:flex;align-items:center;gap:10px;padding:9px 16px;border-bottom:1px solid var(--border);
            ...
            <div style="font-size:.8rem;font-weight:700;font-family:monospace;color:var(--purple)">
              R$ ${fmt((i.qtdAprovada??i.qtdSelecionada)*(i.precoUnitFinal||i.precoUnitEstimado||0))}
            </div>
          </div>`).join('')}
      </div>`;
    }).join('')}
    ...
    <div class="card" style="margin-bottom:14px;overflow:hidden">
      <div style="padding:12px 16px;background:var(--orange-light);border-bottom:1.5px solid var(--border);
        ...
        <div>
          ...
          <div style="font-size:.68rem;color:var(--muted)">Defina local e responsável por item</div>
        </div>
      ...
      ${itensPresencial.map((i, idx) => `
        <div style="padding:12px 16px;border-bottom:1px solid var(--border);background:${idx%2===0?'var(--surface)':'var(--surface2)'}">
...
function marcarOCEnviada(idx) {
  ...
  if (!entry) return;
  entry.itens.forEach(i => { i.statusOC = 'enviada'; });
  saveListas();
}

function _montaOCText(sup, itens, l) {
  const linhas = itens.map(i =>
    `• ${i.nome}: ${fmt(i.qtdAprovada??i.qtdSelecionada)} ${i.unidade} × R$${fmt(i.precoUnitFinal||i.precoUnitEstimado||0)} = R$${fmt((i.qtdAprovada??i.qtdSelecionada)*(i.precoUnitFinal||i.precoUnitEstimado||0))}`
  ).join('\n');
  const total = itens.reduce((s,i) => s+(i.qtdAprovada??i.qtdSelecionada)*(i.precoUnitFinal||i.precoUnitEstimado||0), 0);
...
function imprimirListaPresencial() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
    <title>Lista Presencial ${l.codigo}</title>
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      th{background:#f0f0f0;padding:7px 10px;text-align:left;font-size:11px;border:1px solid #ddd}
      td{padding:8px 10px;border:1px solid #ddd;font-size:12px;vertical-align:top}
      tr:nth-child(even) td{background:#fafafa}
      .footer{margin-top:20px;font-size:11px;color:#666;border-top:1px dashed #ccc;padding-top:12px}
    ...
    <table>
      <thead><tr><th>#</th><th>Item</th><th>Quantidade</th><th>Un.</th><th>Local sugerido</th><th>Teto (R$)</th><th>Obs.</th></tr></thead>
      <tbody>
        ${itens.map((i,idx) => `<tr>
          <td>${idx+1}</td>
          <td><strong>${i.nome}</strong><br><small style="color:#888">${i.categoria}</small></td>
          <td style="text-align:center">${fmt(i.qtdAprovada??i.qtdSelecionada)}</td>
          <td style="text-align:center">${i.unidade}</td>
          <td>${i.localCompra || '—'}</td>
          <td style="text-align:right">R$ ${fmt((i.qtdAprovada??i.qtdSelecionada)*(i.precoUnitEstimado||0))}</td>
          <td>${i.observacoes || ''}</td>
        </tr>`).join('')}
      </tbody>
    </table>
    <div class="footer">
      Responsável: ${l.itens.find(i=>i.responsavelCompra)?.responsavelCompra || '_____________________'}
  ...
  document.getElementById('comprasContent').innerHTML = `
    ...
    <div style="margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;margin-bottom:5px">
        <span style="font-size:.72rem;color:var(--muted)">Progresso do recebimento</span>
    ...
    <!-- Dados globais do recebimento -->
    <div style="background:var(--surface2);border:1.5px solid var(--border);border-radius:var(--r10);padding:14px;margin-bottom:16px">
      <div style="font-size:.69rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);margin-bottom:10px;display:flex;align-items:center;gap:5px">
        ${lc('clipboard-list', 12, 'var(--muted)')} Dados do recebimento
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">
        <div class="field" style="margin:0">
          <label>Quem conferiu</label>
          <input type="text" class="inp" placeholder="Nome do conferente" value="${l.conferidoPor||''}"
            onchange="_setRecebimentoGlobal('conferidoPor',this.value)">
        </div>
        <div class="field" style="margin:0">
          <label>Data de recebimento</label>
          <input type="date" class="inp" value="${l.dataRecebimento||new Date().toISOString().slice(0,10)}"
            onchange="_setRecebimentoGlobal('dataRecebimento',this.value)">
        </div>
        <div class="field" style="margin:0">
          <label>Horário</label>
          <select class="inp" onchange="_setRecebimentoGlobal('horaRecebimento',this.value)">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${l.itens.map(i => {
        ...
        return `<div style="border:1.5px solid ${isOk?'var(--green)':isDivg?'var(--yellow)':'var(--border)'};
          border-radius:var(--r10);background:${isOk?'var(--green-light)':isDivg?'var(--yellow-light)':'var(--surface)'};overflow:hidden">
          <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;flex-wrap:wrap">
            ...
            <div style="flex:1;min-width:130px">
              ...
              <div style="font-size:.66rem;color:var(--muted);margin-top:2px">
                ${i.tipoCompra==='presencial'
                  ? `${lc('shopping-cart',11,'var(--muted)')} ${i.localCompra||'Presencial'}`
                  : `${lc('building-2',11,'var(--muted)')} ${sup?.name||'Fornecedor'}`}
              </div>
            </div>
            <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
              <div style="text-align:center">
                <div style="font-size:.58rem;color:var(--muted);text-transform:uppercase">Solicitado</div>
              ...
              <label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:.76rem;font-weight:600;white-space:nowrap">
                <input type="checkbox" ${i.conferido?'checked':''} style="accent-color:var(--green);width:16px;height:16px"
...
function _setRecebimentoGlobal(campo, val) {
...
function concluirLista() {
  ...
  cycleHistory.push({
    id:              _listaAtual.codigo,
    date:            _listaAtual.dataConclusao,
    items:           _listaAtual.itens.length,
    sups:            Object.keys(bySup).filter(k=>k!=='0').length,
    total:           _listaAtual.valorFinal,
    economia,
    listaId:         _listaAtual.id,
    etapas: {
      criacao:   _listaAtual.dataCriacao,
      aprovacao: _listaAtual.dataAprovacao || null,
      conclusao: _listaAtual.dataConclusao,
    },
    conferidoPor:    _listaAtual.conferidoPor || '',
    dataRecebimento: _listaAtual.dataRecebimento || '',
    horaRecebimento: _listaAtual.horaRecebimento || '',
  });
...
function _renderHistorico() {
  ...
  document.getElementById('comprasContent').innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">
      <input type="text" id="histBusca" class="inp" placeholder="Buscar por código..." value="${busca}"
        style="flex:1;min-width:160px;max-width:260px" oninput="_renderHistorico()">
      <select id="histFiltro" class="inp" style="max-width:180px" onchange="_renderHistorico()">
        <option value="">Todos os status</option>
        ${Object.entries(STATUS_ETAPA).map(([k,v]) =>
          `<option value="${k}" ${filtro===k?'selected':''}>${v.label}</option>`
        ).join('')}
      </select>
    </div>
    ...
    ${hist.length === 0 ? `
      <div class="empty" style="padding:40px">
        <div class="empty-icon">${lc('clock', 24, 'var(--muted)')}</div>
  ...
  return `<div class="card" style="margin-bottom:12px;overflow:hidden">
    <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:var(--surface2);border-bottom:1px solid var(--border);flex-wrap:wrap">
      ...
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
        <div style="text-align:center">
          ...
          <div style="font-size:.56rem;color:var(--muted);text-transform:uppercase">Itens</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:.82rem;font-weight:800;color:var(--purple)">${numSups}</div>
          <div style="font-size:.56rem;color:var(--muted);text-transform:uppercase">Fornec.</div>
        </div>
        <div style="text-align:center">
          <div style="font-size:.82rem;font-weight:800;color:${l.status==='concluida'?'var(--green)':'var(--purple)'}">
            R$${fmt(l.valorFinal||l.valorEstimado||0)}
          </div>
          <div style="font-size:.56rem;color:var(--muted);text-transform:uppercase">${l.status==='concluida'?'Final':'Estimado'}</div>
        </div>
    ...
    <div style="padding:14px 16px;background:var(--surface)">
      <div style="position:relative;display:flex;align-items:flex-start">
        ...
        ${etapas.map((e, idx) => {
          const done = !!e.data;
          const cur  = !done && idx === etapas.filter(x=>!!x.data).length;
          return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;position:relative;z-index:1">
            <div style="width:22px;height:22px;border-radius:50%;
              background:${done?'var(--green)':cur?'var(--purple)':'var(--border)'};
              border:2px solid ${done?'var(--green)':cur?'var(--purple)':'var(--border)'};
              display:flex;align-items:center;justify-content:center;margin-bottom:5px">
              ${lc(e.icon, 10, done||cur?'#fff':'var(--muted)')}
            </div>
            <div style="font-size:.58rem;font-weight:600;color:${done?'var(--green)':cur?'var(--purple)':'var(--muted)'};text-align:center;line-height:1.3">
              ${e.label}
            </div>
    ...
    <div style="padding:8px 16px;border-top:1px solid var(--border);display:flex;flex-wrap:wrap;gap:5px;align-items:center">
      ${numSups > 0 ? `<span style="font-size:.6rem;color:var(--muted)">${lc('building-2',10,'var(--muted)')} Fornecedores:</span>` : ''}
      ${Object.keys(bySup).filter(k=>k!=='0').map(k => {
        const s = suppliers.find(x=>x.id===parseInt(k));
        return s ? `<span class="badge b-purple" style="font-size:.58rem">${s.name}</span>` : '';
      }).join('')}
...
function encerrarListaManual() {
  ...
  _listaAtual.status        = 'concluida';
  _listaAtual.dataConclusao = new Date().toISOString();
  _listaAtual.valorFinal    = _listaAtual.valorEstimado || 0;
  saveListas();
  _listaAtual = getListaAtiva();
  renderComprasModule();
...
function calcEconomia() {
  return listas.filter(l => l.status === 'concluida')
    .reduce((s,l) => s + Math.max(0,(l.valorEstimado||0)-(l.valorFinal||0)), 0);
}
